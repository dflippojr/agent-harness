...
- Images (`images:`; `docs/phase6d-results.md`): ComfyUI (`C:\AI\ComfyUI`, started on demand) with Z-Image-Turbo (`fast`)
  ...
  Phone: Images screen; agents: `generate_image` (tower and MacBook sessions). Optional `image_edit` adds owner-only
  masked inpainting with Qwen-Image-Edit; it is not downloaded unless that component is enabled.
