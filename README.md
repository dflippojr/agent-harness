...
- Images (`images:`; `docs/phase6d-results.md`): ComfyUI (`C:\AI\ComfyUI`, started on demand) with Z-Image-Turbo (`fast`)
  ...
  preserves the original PNG. Optional `image_edit` adds owner-only masked inpainting with Qwen-Image-Edit; it is
  not downloaded unless that component is enabled (`docs/INSTALL.md`).
